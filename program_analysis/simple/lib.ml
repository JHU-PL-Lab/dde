    ...
    | App (e, _, l) -> (
        ...
        | _ ->
            ...
            if Set.mem v v_state then (
              ...
              debug (fun m ->
                  m "Evaluating applied function: %a" Interp.Pp.pp_expr e);
              ...
              let%bind r2 =
                Set.fold r1 ~init:(return empty_res) ~f:(fun acc a ->
                    ...
                    | FunAtom (Fun (_, e1, _), _) ->
                        debug (fun m ->
                            m
                              "[App] Evaluating body of applied function \
                               applied: %a"
            ...
            if Set.mem v est then (
              ...
              | App (e1, e2, l) ->
                  if idx = 0 then (
                    ...
                    debug (fun m -> m "S: %a" S.pp s);
                    (* Stitch the stack to gain more precision *)
                    let sufs = suffixes l sigma_tl s in
                    let%bind r1 =
                      List.fold sufs ~init:(return empty_res) ~f:(fun acc suf ->
                          debug (fun m ->
                              m
                                "[Level %d][Var Non-Local] Stitched! Evaluate \
                                 %a using stitched stack %a"
