...
let rec analyze_aux ~caching d expr sigma pi : Res.t T.t =
  ...
  | App (e, _, l) -> (
      ...
      | _ ->
          ...
          if Set.mem v v_state then (
            ...
            in
            let r2 =
              simplify (elim_stub (Set.elements r2) (St.Lstate cycle_label))
            in
  ...
  | If (e, e_true, e_false) -> (
      ...
      | true, false when not any_timeout ->
          ...
          [ PathCondAtom (pc_true, r_true) ] |> simplify |> return
      | false, true when not any_timeout ->
          ...
          info (fun m -> m "[Level %d] *** If False only ***" d);
          info (fun m -> m "[Level %d] *** If ***" d);
          [ PathCondAtom (pc_false, r_false) ] |> simplify |> return
      ...
      | _ ->
          ...
          [ pc_true; pc_false ] |> simplify |> return)
  (* Operation rule *)
  | Plus (e1, e2)
  | Minus (e1, e2)
  | Mult (e1, e2)
  ...
  | Rec es ->
      ...
      |> fun rs ->
      let%bind rs = rs in
      [ RecAtom (List.rev rs) ] |> simplify |> return
  (* Record Project rule *)
  | Proj (e, x) ->
      let%bind r0 = analyze_aux ~caching d e sigma pi in
      [ ProjAtom (r0, x) ] |> simplify |> return
  (* Record Inspect rule *)
  | Insp (x, e) ->
      let%bind r0 = analyze_aux ~caching d e sigma pi in
      [ InspAtom (x, r0) ] |> simplify |> return
  (* e.g., letassert x = 10 in x >= 0 *)
  | LetAssert (id, e1, e2) ->
      let%bind r1 = analyze_aux ~caching d e1 sigma pi in
      let r2 = eval_assert e2 id in
      [ AssertAtom (id, r1, r2) ] |> simplify |> return
