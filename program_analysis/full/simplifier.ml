...
(** Performs simplifications on value atoms as specified in paper Section 5.2.1 *)
let rec simpl_atom a =
  match a with
  | IntAtom _ | BoolAtom _ | LStubAtom _ | EStubAtom _ | FunAtom _ -> [ a ]
  | LResAtom (r, st) -> (
      match r with
      | [] -> []
      | _ when not (exists_stub_res (St.Lstate st) r) -> r
      | _ -> [ LResAtom (simpl_res r, st) ])
  | EResAtom (r, st) -> (
      match r with
      | [] -> []
      | _ when not (exists_stub_res (St.Estate st) r) -> r
      | _ -> [ EResAtom (simpl_res r, st) ])
  ...
  | MultAtom (r1, r2) -> (
      ...
      | [ IntAtom i1 ], [ MultAtom ([ IntAtom i2 ], r2) ] ->
          [ MultAtom ([ IntAtom (i1 * i2) ], r2) ]
      | [ IntAtom i1 ], [ EResAtom ([ IntAtom i2 ], st) ]
      | [ EResAtom ([ IntAtom i1 ], st) ], [ IntAtom i2 ] ->
          [ EResAtom ([ IntAtom (i1 * i2) ], st) ]
      | _ -> [ MultAtom (simpl_res r1, simpl_res r2) ])
  | EqAtom (r1, r2)
  | GeAtom (r1, r2)
  | GtAtom (r1, r2)
  | LeAtom (r1, r2)
  | LtAtom (r1, r2) -> (
      match (r1, r2) with
      | [ IntAtom i1 ], [ IntAtom i2 ] -> [ BoolAtom (int_cmp_op a i1 i2) ]
      | _ ->
          let r1', r2' = (simpl_res r1, simpl_res r2) in
          [
            (match a with
  ...
  | NotAtom r -> (
      ...
      | _ -> r |> simpl_res |> List.map ~f:(fun a -> NotAtom [ a ]))
  | AssertAtom (id, r, rv) -> [ AssertAtom (id, simpl_res r, rv) ]
  ...
  | RecAtom rs ->
      [ RecAtom (List.map rs ~f:(fun (id, r) -> (id, simpl_res r))) ]
...
(** Performs simplifications on value results as specified in paper Section 5.2.1 *)
and simpl_res r =
  r
  |> List.concat_map ~f:simpl_atom
