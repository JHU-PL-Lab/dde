  ...
  if exists_stub r label then
    ...
    let bases =
      List.filter_map r ~f:(fun a ->
          match a with
          | RecordAtom _ when not (exists_stub [ a ] label) -> Some a
...
let rec analyze_aux_step ?(should_cache = true) d expr sigma pi s v v' =
      ...
      let r, s' =
        ...
        | Appl (e, _, l) ->
            ...
            if Set.mem v lst then (
              ...
              let r1, s1 =
                analyze_aux_step ~should_cache d e sigma pi s new_v new_v'
              in
              ...
              let r2, s2 =
                fold_res_app ~init:(empty_choice_set, new_s) l sigma d r1
                  ~f:(fun (acc_r, acc_s) e1 ->
                    ...
                    let ri, si =
                      analyze_aux_step ~should_cache d e1 pruned_sigma' pi new_s
                        new_v new_v'
                    in
                          ...
                          Set.fold s ~init:(empty_choice_set, s)
                            ~f:(fun (acc_r, acc_s) sigma_i ->
                              ...
                              then (
                                ...
                                let ri, si =
                                  analyze_aux_step ~should_cache d e2 sigma_i_tl
                                    pi s new_v new_v'
                                in
                            ...
                            ~f:(fun (acc_r, acc_s) si ->
                              ...
                              then (
                                ...
                                (* let new_v =
                                   ...
                                   in *)
                                let r0, s0 =
                                  analyze_aux_step ~should_cache d e1 si_tl pi s
                                    v v'
                                in
                              ...
                              if Stdlib.(x1 = x1') && l_myfun = l1 then (
                                debug (fun m ->
                                    m
                                      ...
                                      x l1);
                                let r0', s0' =
                                  analyze_aux_step ~should_cache d
                                    (Var (Ident x, l1))
        ...
        | If (e, e_true, e_false, l) -> (
            ...
            let d = d + 1 in
            let r_cond, s_cond =
              analyze_aux_step ~should_cache d e sigma None s v v'
            in
            ...
            | [ BoolAtom b ] ->
                if b then
                  let r_true, s_true =
                    analyze_aux_step ~should_cache d e_true sigma None s v v'
                  in
                  (r_true, Set.union s_cond s_true)
                else
                  let r_false, s_false =
                    analyze_aux_step ~should_cache d e_false sigma None s v v'
                  in
        ...
        | Lt (e1, e2) ->
            ...
            let r1, s1 = analyze_aux_step ~should_cache d e1 sigma pi s v v' in
            let r2, s2 = analyze_aux_step ~should_cache d e2 sigma pi s1 v v' in
            debug (fun m ->
        ...
        | Not e -> (
            let d = d + 1 in
            let r, s = analyze_aux_step ~should_cache d e sigma pi s v v' in
            match r with
            | [ BoolAtom b ] ->
                (* Format.printf "%a\n" pp_res r; *)
                ([ BoolAtom (not b) ], s)
            ...
            ( [
                RecordAtom
                  ...
                  |> List.map ~f:(fun (x, ei) ->
                         ( x,
                           let r, _ =
                             analyze_aux_step ~should_cache d ei sigma pi s v v'
                           in
                           r )));
              ],
              s )
        | Projection (e, Ident key) ->
            ...
            let d = d + 1 in
            let r0, s0 = analyze_aux_step ~should_cache d e sigma pi s v v' in
...
and analyze_aux ?(should_cache = true) n e_glob d expr sigma pi s v v' =
  debug (fun m -> m "[Level %d] [n = %d] Entered analyze_aux" d n);
  let r, s' = analyze_aux_step ~should_cache d expr sigma pi s v v' in
  ...
  if Set.compare_direct s s' = 0 then (
    ...
    (* Hashtbl.clear cache; *)
    let r, s' =
      analyze_aux (n + 1) e_glob 0 e_glob [] None s'
