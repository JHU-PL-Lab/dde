...
let rec eval_helper ?(is_lazy = false) le (sigma : int list option) =
  ...
  | None ->
      ...
      let eval_res =
        ...
        | Var (Ident x) -> (
            ...
            | Function (Ident x', _) -> (
                let sigma : int list = Option.get sigma in
                if x = x' then
                  (* Var Local *)
...
let rec label_to_expr target_label (target : lexpr) sigma seen : lexpr =
