...
- ... 1 takes ~10 seconds, 1 takes ~20 seconds, 3 time out after 10 minutes, and
  ...
  (`sat-1`, `sat-2`, `sat-3`, `ack`, `tak`, `cpstak`) (TODO: confirm). The same
  3 cannot be expressed (`flatten`, `deriv`, `regex`).
...
Beware that `z3` takes a long time and much compute power to install. For this
...
Graphviz, which gives you the `dot` command.
...
only used to generate visualizations for the paper.
...
You may find comments throughout the source code to aid you with understanding
and reusing the various components.
